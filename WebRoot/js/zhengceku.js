...
$(function(){
	...
	layui.use('laypage', function(){
		...
		window.fzFilter = function(){
			...
			$.ajax({
			    type: "post",
			    data: {'pageNum':1,'pageSize':10,'timeStageId':tsi,'areaFlagId':areaFlagId,'areaId':areaId,'zoneId':zoneId,'businessPeopleId':businessPeopleId,'startDate':startDate,'endDate':endDate,'industryId':industryId,'unitName':unitId,'issuedno':issuedno},
			    url: 'selectPolicyIssuedByZCCXClick.html',
			    dataType: "json",
			    beforeSend: function(xhr) {
			    	$('#zck-zc-list').empty();
			    	$('#zck-zc-list').append('<img src="/imgs/huanchong.gif" id="huanchong">');
			    	$('#zck-fenye').show();
			    },
			    success: function(res) {
							var data = res.data;
							if(data.length==0){
		                      $('#zck-zc-list').append('<div style="text-align:center;margin-top:30px;padding-bottom:30px;"><img src="/imgs/no_data_icon.png" style="display:inline-block;"></div>');
		                      $('#zck-fenye').hide();
		                      return;
		                    }
							for (var i = 0; i < data.length; i++) {
								var shortContent = ellipsis(data[i].policyIntroduction,200);
								var html =  '<li>'+					
												'<a href="skipPolicyDetails.html?id='+data[i].id+'" class="title" target="_blank">'+ data[i].name +'</a>'+
												'<div class="content">'+								
													'<div class="detail">'+
														'<p class="p1">'+ shortContent +'</p>'+
														'<p class="p2"><img src="/imgs/list-time.png"><em>发布时间：'+ data[i].reportDate +'</em><span>发布单位：<i>'+ data[i].region +'</i></span><p>'+
													'</div>'+
												'</div>'+					
											'</li>';
								$('#zck-zc-list').append(html);
							}
							var pageCounts = res.allCounts;
							//执行一个laypage实例
							laypage.render({
							    ...
							    jump: function(obj, first){
								   ...
								   // console.log(obj.limit); //得到每页显示的条数							   
								    ...
								    if(!first){
								        $.ajax({
									        type: "post",
									        data: {'pageNum':obj.curr,'pageSize':obj.limit,'timeStageId':tsi,'areaFlagId':areaFlagId,'areaId':areaId,'zoneId':zoneId,'businessPeopleId':businessPeopleId,'startDate':startDate,'endDate':endDate,'industryId':industryId,'unitName':unitId,'issuedno':issuedno},
			                                url: 'selectPolicyIssuedByZCCXClick.html',
									        dataType: "json",
									        beforeSend: function(xhr) {
									        	$('#zck-zc-list').empty();
        ...
        window.kwSearch = function(kw){        
        	$.ajax({
			    ...
			    url: 'http://localhost:8080/front/selectPolicyIssuedByZCCXClick.html',
			    dataType: "json",
			    beforeSend: function(xhr) {
			    	$('#zck-zc-list').empty();
			    	$('#zck-zc-list').append('<img src="/imgs/huanchong.gif" id="huanchong">');
	...
	$.ajax({
	    ...
	    success: function(res) {
				//console.log(res)
					...
					//区域政策
						...
						for (var i = 0; i < newAreaArray.length; i++) {
					    	...
					    	for (var j = 0; j < newAreaArray[i].data.length; j++) {				
					    		$('#zck-city-list li').eq(i).append('<a href="javascript:;" id="'+ newAreaArray[i].data[j].id +'" class="filter-item-a" data-text="'+ newAreaArray[i].data[j].name +'">'+ newAreaArray[i].data[j].name +'</a>');				    		;
					    	}
					    };
				    //区域政策点击
				    ...
				    //政策分类
				    data = res.policyClassification;
				    for (var i = 0; i < data.length; i++) {
						var html = '<a href="javascript:;" id="'+ data[i].id +'" class="filter-item-a" data-text="'+ data[i].name +'">'+ data[i].name +'</a>';
					...
					data = res.industry;
				    for (var i = 0; i < data.length; i++) {
						var html = '<a href="javascript:;" id="'+ data[i].id +'" class="hangye-label">'+ data[i].name +'</a>';
